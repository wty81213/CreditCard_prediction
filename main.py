...
# datasetloader = split_df
# cat_feats = category_feat
...
#     data=trainset['X'].values,
#     label=trainset['y'],
#     feature_name=trainset['X'].columns.tolist(),
...
# all_trainset = {
#     'X': pd.concat([trainset['X'], validset['X']], axis=0),
#     'y': np.append(trainset['y'], validset['y'])
# }

# lgbtrain = lgb.Dataset(
#     data=all_trainset['X'].values,
#     label=all_trainset['y'],
#     feature_name=all_trainset['X'].columns.tolist(),
#     categorical_feature=cat_feats,
#     free_raw_data=False)

# F_model = lgb.train(best_param, lgbtrain, num_boost_round=300)
# predict_test_y = (F_model.predict(testset['X']) >= 0.5).astype('int')
# submit_table = pd.DataFrame({
#     'txkey': testset['X'].index,
#     'fraud_ind': predict_test_y
# })
